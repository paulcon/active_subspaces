...
# Shift and scale inputs from [-1,1] hypercube to describe ranges
    ...
    Sw      = (xx[0]+1)*0.5*(200-150)+150;
    Wfw     = (xx[1]+1)*0.5*(300-220)+220;
    A       = (xx[2]+1)*0.5*(10-6)+6;
    LamCaps = (xx[3]+1)*0.5*(Lub-Llb)+Llb;
    q       = (xx[4]+1)*0.5*(45-16)+16;
    lam     = (xx[5]+1)*0.5*(1-0.5)+0.5;
    tc      = (xx[6]+1)*0.5*(0.18-0.08)+0.08;
    Nz      = (xx[7]+1)*0.5*(6-2.5)+2.5;
    Wdg     = (xx[8]+1)*0.5*(2500-1700)+1700;
    Wp      = (xx[9]+1)*0.5*(0.08-0.025)+0.025;
    ...
    dy = np.array([ 
        ...
        **0.49E0*Wfw**0.35E-2*(tc*(1/np.cos(LamCaps)))**(-0.3E0)*(A*(1/np.cos(LamCaps))**2)**0.6E0,
        #dy/dWfw
        0.316498E-4*lam**0.4E-1*q**0.6E-2*Sw**0.758E0*(Nz*Wdg)  \
        **0.49E0*Wfw**(-0.9965E0)*(tc*(1/np.cos(LamCaps)))**(-0.3E0)*(A*(1/np.cos(LamCaps))**2)**0.6E0,
        #dy/dA
        0.542567E-2*lam**0.4E-1*q**0.6E-2*Sw**0.758E0*(Nz*Wdg)  \
        **0.49E0*Wfw**0.35E-2*(1/np.cos(LamCaps))**2*(tc*(1/np.cos(LamCaps)))**(  \
        -0.3E0)*(A*(1/np.cos(LamCaps))**2)**(-0.4E0),
        #dy/dLamCaps
        0.108513E-1*A*lam**0.4E-1*q**0.6E-2*Sw**0.758E0*(Nz*Wdg)  \
        **0.49E0*Wfw**0.35E-2*(1/np.cos(LamCaps))**2*(tc*(1/np.cos(LamCaps)))**(  \
        -0.3E0)*(A*(1/np.cos(LamCaps))**2)**(-0.4E0)*np.tan(LamCaps)+(  \
        -0.271284E-2)*lam**0.4E-1*q**0.6E-2*Sw**0.758E0*tc*(Nz*Wdg)  \
        **0.49E0*Wfw**0.35E-2*(1/np.cos(LamCaps))*(tc*(1/np.cos(LamCaps)))**(  \
        -0.13E1)*(A*(1/np.cos(LamCaps))**2)**0.6E0*np.tan(LamCaps),
        #dy/dq
        ...
        **0.49E0*Wfw**0.35E-2*(1/np.cos(LamCaps))*(tc*(1/np.cos(LamCaps)))**(  \
        -0.13E1)*(A*(1/np.cos(LamCaps))**2)**0.6E0,
        #dy/dNz
        0.443097E-2*lam**0.4E-1*q**0.6E-2*Sw**0.758E0*Wdg*(Nz*Wdg)  \
        **(-0.51E0)*Wfw**0.35E-2*(tc*(1/np.cos(LamCaps)))**(-0.3E0)*(A*(1/np.cos(LamCaps))**2)**0.6E0,
        #dy/dWdg
        0.443097E-2*lam**0.4E-1*Nz*q**0.6E-2*Sw**0.758E0*(Nz*Wdg)**(  \
        -0.51E0)*Wfw**0.35E-2*(tc*(1/np.cos(LamCaps)))**(-0.3E0)*(A*(1/np.cos(LamCaps))**2)**0.6E0,
        #dy/dWp
        Sw])
    scaling = np.array([50,80,4,Lub-Llb,45-16,0.5,0.10,3.5,800,0.08-0.025])*0.5
    dy = dy*scaling
    return [y,dy]
